...
#ifndef CLIP
#define CLIP(x, lb, ub) (MAX(MIN((x), (ub)), (lb)))
#endif
...
inline dtype RightShiftRoundUp(dtype val, dtype bits) {
  if (bits <= 0) {
    return val;
  }
  else {
...
template<typename dtype>
inline dtype Clip(dtype val, dtype lb, dtype ub) {
  if (val < lb) {
    return lb;
  } else if (val > ub) {
    return ub;
...
struct TSize {
  ...
  TSize(T w, T h) { this->w = w; this->h = h; }

  TSize<T>& operator = (const TSize<T> &size) { this->w = size.w; this->h = size.h;  return *this; }
};
